...
class PaaCreater: NSObject {
    
    class func randomData() -> NSString? {
        
        //商户订单金额、以分为单位
        var amount = arc4random() % 50000 + 1
        
        //商户订单生成日期
        var dateFormatter = NSDateFormatter();
        dateFormatter.dateFormat = "yyyyMMddHHmmss"
        var workDate = NSDate(timeIntervalSinceReferenceDate: NSDate.timeIntervalSinceReferenceDate())
        var timeStr = dateFormatter.stringFromDate(workDate)
        
        //商户订单号
        var orderStr = NSString(format: "%@%@", timeStr, "0000")
        
//####################################  计算外卡扩展信息字段签名  ######################################//
        
//        var ext1 = ext1From(extPart())
        
//##################################################################################################//
        
        var paaArr: Array<String> = [
...
//            "3", "language",
            ...
            //通联分配给商户的ID
            "100020091218001", "merchantId",//必填
            
            //商户当前支付订单号
            orderStr as String, "orderNo",//针对最新语法修改
...
//############################  外卡支付订单数据与签名请参考以下部分  ###################################//
    ...
    //外卡扩展字段示例数据
    class func extPart() -> Dictionary<String, String> {
    
        var tmpDic: Dictionary<String, String> = [
...
//            "cvv2":""
    ...
    //计算外卡扩展字段的签名信息
    class func ext1From(extPart: Dictionary<String, String>) -> String {
    
        if (extPart.isEmpty) { return "" }
    
        var keys: Array<String> = [ "customer_id", "customer_first_name", "customer_last_name", "customer_email",
                                    ...
                                    "passengertype", "passenger_status", "ship_to_country", "ship_to_state", "ship_to_city", "ship_to_street1", "ship_to_street2", "ship_to_phonenumber", "ship_to_postalcode", "ship_to_firstname",
        ...
        for key in keys {
            ...
            if (extPart[key] != nil) { mstring.extend(extPart[key]!) }
        }
    
        if (mstring.isEmpty) { return "" }
    
        var ext1 = md5(mstring as NSString)
    
        return ext1 as! String
    }
...
//##################################################################################################//
    ...
    **/
    class func formatPaa(array: Array<String>) -> NSString? {
        
        var mdic = NSMutableDictionary(capacity: 1)
        
        var paaStr = NSMutableString()
        for var i = 0; i < array.count; ++i {
            let next = i + 1
            paaStr.appendFormat(NSString(format: "%@=%@&", array[next], array[i]))
            mdic.setValue(array[i], forKey: array[next])
            i = next
        }
        
        let length = paaStr.length
        let signMsg: NSString? = md5(paaStr.substringToIndex(length - 1))
        mdic.setValue(signMsg?.uppercaseString, forKey: "signMsg")
...
//############################  卡号回显请参考以下部分  ##############################################//
        ...
        //该字段不参与订单信息的签名计算
        //若不需要卡号回显的功能该字段可不传入
//        let cardNo = "4391880000000004"//测试用信用卡
//                   "6225882120759623"//测试用借记卡
//        mdic.setValue(cardNo, forKey: "cardNo")
        
//##################################################################################################//
        
//############################  外卡支付订单添加扩展信息字段  ###########################################//
        
//        mdic.addEntriesFromDictionary(extPart())
        
//##################################################################################################//
        
        if (mdic.valueForKey("key") != nil) {//商户私有签名密钥 通联后台持有不传入插件
            mdic.removeObjectForKey("key")
        }
        
        var error: AutoreleasingUnsafeMutablePointer<NSError?> = nil
        let data: NSData? = NSJSONSerialization.dataWithJSONObject(mdic, options: NSJSONWritingOptions.PrettyPrinted, error: error)
        let jsonStr: NSString = NSString(data: data!, encoding: NSUTF8StringEncoding)!
    ...
    **/
    ...
    //计算字符串对应的md5值
    class func md5(string: NSString) -> NSString? {
